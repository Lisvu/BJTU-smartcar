...
printf '%s\n' "$holders"
holder_pids="$(run_sudo fuser /dev/video0 /dev/rplidar /dev/myserial 2>/dev/null || true)"
if [ -n "${holder_pids//[[:space:]]/}" ]; then
  log "one or more devices still have holders: $holder_pids"
  exit 2
fi

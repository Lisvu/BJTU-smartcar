...
class Rosmaster(object):
    ...
    def __parse_data(self, ext_type, ext_data):
        if self.__debug:
            if time.time() - self.time_4_log >= 0.5:
                self.time_4_log = time.time()
                print("ext_type:"+str(ext_type))
                hex_cmd = [hex(num) for num in ext_data]
                print("data:", hex_cmd)
        ...
        if ext_type == self.FUNC_REPORT_SPEED:
            ...
            self.data_queue.put([self.__vx, self.__vy, self.__vz])
    ...
    def __request_data(self, function, param=0):
        ...
        self.ser.write(cmd)
        if self.__debug:
            hex_cmd = [hex(num) for num in cmd]
            print("request:", hex_cmd)
        time.sleep(0.002)
    ...
    # 巡线功能开关，flag=0：关闭，flag=1：打开，
    def set_follow_line(self, flag):
        try:
            cmd = [self.__HEAD, self.__DEVICE_ID, 0x04, self.FUNC_FOLLOW_LINE, flag]
            checksum = sum(cmd, self.__COMPLEMENT) & 0xff
            cmd.append(checksum)
            if self.__debug:
                print(cmd)
            ...
            if self.__debug:
                hex_cmd=[hex(num) for num in cmd]
                print("follow the line:", hex_cmd)
            time.sleep(self.__delay_time)
        except:
            print('---set_follow_line error!---')
            pass

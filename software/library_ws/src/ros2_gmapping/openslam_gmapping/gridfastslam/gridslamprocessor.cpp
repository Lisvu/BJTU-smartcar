...
(double srr, double srt, double str, double stt){
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){
    ...
    // 在粒子滤波过程中考虑IMU数据
    if(m_matcher.getUseIMU()) {
        // 获取IMU方向
        double imuOrient = m_matcher.getIMUOrientation();
        double imuWeight = m_matcher.getIMUWeight();
        ...
        for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
            ...
            // 调整粒子的方向，使其更接近IMU方向
            double angleDiff = imuOrient - pose.theta;
            // 确保差异在[-π, π]范围内
            while(angleDiff > M_PI) angleDiff -= 2*M_PI;
            while(angleDiff < -M_PI) angleDiff += 2*M_PI;
            
            // 根据IMU权重调整粒子方向
            pose.theta += imuWeight * angleDiff;
            pose.theta = atan2(sin(pose.theta), cos(pose.theta)); // 归一化角度
        }
    }

...
OrientedPoint 
MotionModel::drawFromMotionWithIMU(const OrientedPoint& p, const OrientedPoint& pnew, 
    ...
    if (useIMU) {
        // 提取里程计估计的角度变化
        double odometryDTheta = delta.theta + sampleGaussian(stt*fabs(delta.theta) + 
                                                           srt*sqrt(delta.x*delta.x + delta.y*delta.y));
        
        // 计算 IMU 的角度变化（相对于上一帧）
        double imuDTheta = atan2(sin(imuOrientation - p.theta), cos(imuOrientation - p.theta));
        
        // 融合 IMU 和里程计的角度估计
        noisypoint.theta = (1.0 - imuAngularWeight) * odometryDTheta + imuAngularWeight * imuDTheta;
    } else {
        // 只使用里程计估计
        noisypoint.theta += sampleGaussian(stt*fabs(delta.theta) + 
                                         srt*sqrt(delta.x*delta.x + delta.y*delta.y));
    }

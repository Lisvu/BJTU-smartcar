  ...
  */
   ...
   */
    ...
    */
    ...
    void setMotionModelIMU(bool useIMU, double weight) {
        m_motionModel.useIMU = useIMU;
        m_motionModel.imuAngularWeight = weight;
    }
    
    void setIMUOrientation(double orientation) {
        m_imuOrientation = orientation;
    }

    // 添加对IMU数据的支持方法
    inline void setUseIMU(bool useIMU) { m_matcher.setUseIMU(useIMU); }
    inline void setIMUWeight(double weight) { m_matcher.setIMUWeight(weight); }
    inline void updateIMUOrientation(double orientation) { m_matcher.updateIMUOrientation(orientation); }

...
ScanMatcher::ScanMatcher(): m_laserPose(0,0,0), m_generateMap(false), m_activeAreaComputed(false){
	//m_laserAngles=0;
	m_laserBeams=0;
	m_optRecursiveIterations=3;
	m_useIMU = false;
	m_imuWeight = 0.5;
	m_imuOrientation = 0.0;
...
// double ScanMatcher::registerScan(ScanMatcherMap& map, const OrientedPoint& p, const double* readings, double imuOrientation) {
// 	// ±£´æÔ­Ê¼IMU×´Ì¬
// 	bool originalUseIMU = m_useIMU;
// 	double originalOrientation = m_imuOrientation;
	
// 	// ÉèÖÃIMU²ÎÊý
// 	m_useIMU = true;
// 	m_imuOrientation = imuOrientation;
	
// 	// µ÷ÓÃÔ­Ê¼registerScan·½·¨
// 	double result = registerScan(map, p, readings);
	
// 	// »Ö¸´Ô­Ê¼IMU×´Ì¬
// 	m_useIMU = originalUseIMU;
// 	m_imuOrientation = originalOrientation;
...
// double ScanMatcher::optimize(OrientedPoint& pnew, const ScanMatcherMap& map, const OrientedPoint& init, const double* readings, double imuOrientation) const {
// 	// ÁÙÊ±±£´æÔ­Ê¼IMUÖµ
// 	bool originalUseIMU = m_useIMU;
// 	double originalOrientation = m_imuOrientation;
	
// 	// ÉèÖÃIMU²ÎÊý
// 	const_cast<ScanMatcher*>(this)->m_useIMU = true;
// 	const_cast<ScanMatcher*>(this)->m_imuOrientation = imuOrientation;
	
// 	// µ÷ÓÃÔ­Ê¼µÄÓÅ»¯·½·¨
// 	double result = optimize(pnew, map, init, readings);
	
// 	// »Ö¸´Ô­Ê¼IMUÖµ
// 	const_cast<ScanMatcher*>(this)->m_useIMU = originalUseIMU;
// 	const_cast<ScanMatcher*>(this)->m_imuOrientation = originalOrientation;
...
double ScanMatcher::optimize(OrientedPoint& pnew, const ScanMatcherMap& map, const OrientedPoint& init, const double* readings) const {
	double bestScore = -1;
	OrientedPoint bestPose;
	
	// Èç¹ûÊ¹ÓÃIMUÊý¾Ý£¬Ê×ÏÈ¿¼ÂÇIMUÌá¹©µÄ·½Ïò
	if(m_useIMU) {
		OrientedPoint imuPose = init;
		// Ê¹ÓÃIMUÌá¹©µÄ·½Ïò½Ç£¬µ«±£³ÖÔ­Ê¼Î»ÖÃ
		double angleDiff = m_imuOrientation - init.theta;
		while(angleDiff > M_PI) angleDiff -= 2*M_PI;
		while(angleDiff < -M_PI) angleDiff += 2*M_PI;
		
		imuPose.theta = init.theta + m_imuWeight * angleDiff;
		imuPose.theta = atan2(sin(imuPose.theta), cos(imuPose.theta));
		
		double imuScore = score(map, imuPose, readings);
		if(imuScore > bestScore) {
			bestScore = imuScore;
			bestPose = imuPose;
		}
	}
	
	// Èç¹ûÃ»ÓÐÕÒµ½ºÃµÄIMUÆ¥Åä£¬Ê¹ÓÃÔ­Ê¼Î»×Ë
	if(bestScore < 0) {
		bestPose = init;
	}
	...
	OrientedPoint currentPose = bestPose;
	...
	do {
		...
		do {
			...
			double imu_gain = 1.0;
			if (m_useIMU) {
				double dth_imu = m_imuOrientation - localPose.theta;
				dth_imu = atan2(sin(dth_imu), cos(dth_imu));
				dth_imu *= dth_imu;
				imu_gain = exp(-m_imuWeight * dth_imu);
			}
			
			double localScore = odo_gain * imu_gain * score(map, localPose, readings);
	...
	// ËÑË÷×î¼Ñ·½ÏòÊ±¿¼ÂÇIMUÊý¾Ý
	if(m_useIMU) {
		// ËÑË÷½Ç¶ÈÊ±£¬Ôö¼ÓIMU·½ÏòÖÜÎ§µÄËÑË÷ÃÜ¶È
		double imuAngleDiff = fabs(currentPose.theta - m_imuOrientation);
		imuAngleDiff = imuAngleDiff > M_PI ? 2*M_PI - imuAngleDiff : imuAngleDiff;
		
		if(imuAngleDiff < 0.5) { // Èç¹ûIMU·½ÏòÓëµ±Ç°·½Ïò½Ó½ü
			// ËõÐ¡ËÑË÷·¶Î§£¬Ôö¼Ó¾«¶È
			for(double dth = -0.2; dth < 0.2; dth += 0.05) {
				OrientedPoint orientedSample(currentPose.x, currentPose.y, currentPose.theta + dth);
				double odo_gain = 1;
				double scoreValue = odo_gain * score(map, orientedSample, readings);
				
				if(scoreValue > currentScore) {
					currentScore = scoreValue;
					currentPose = orientedSample;
				}
			}
		}
	}
...
void ScanMatcher::setIMUParameters(bool useIMU, double imuWeight) {
	m_useIMU = useIMU;
	m_imuWeight = imuWeight;
}

void ScanMatcher::updateIMUOrientation(double orientation) {
	m_imuOrientation = orientation;
}

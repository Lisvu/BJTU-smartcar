...
void SlamGmapping::init() {
    ...
    // this->get_parameter("useIMU", use_imu_);
    // this->get_parameter("imuAngularWeight", imu_weight_);
    ...
    RCLCPP_INFO(this->get_logger(), "angularUpdate: %f", angularUpdate_);
    RCLCPP_INFO(this->get_logger(), "lstep: %f", lstep_);
    RCLCPP_INFO(this->get_logger(), "astep: %f", astep_);
    RCLCPP_INFO(this->get_logger(), "linearUpdate: %f", linearUpdate_);
    RCLCPP_INFO(this->get_logger(), "temporalUpdate: %f", temporalUpdate_);
    RCLCPP_INFO(this->get_logger(), "maxUrange: %f", maxUrange_);
    RCLCPP_INFO(this->get_logger(), "maxRange: %f", maxRange_);
    RCLCPP_INFO(this->get_logger(), "minimum_score: %f", minimum_score_);
    RCLCPP_INFO(this->get_logger(), "sigma: %f", sigma_);
    RCLCPP_INFO(this->get_logger(), "kernelSize: %d", kernelSize_);
    RCLCPP_INFO(this->get_logger(), "iterations: %d", iterations_);
    RCLCPP_INFO(this->get_logger(), "occ_thresh: %f", occ_thresh_);
    RCLCPP_INFO(this->get_logger(), "particles: %d", particles_);
    RCLCPP_INFO(this->get_logger(), "xmin: %f", xmin_);
    RCLCPP_INFO(this->get_logger(), "ymin: %f", ymin_);
    RCLCPP_INFO(this->get_logger(), "xmax: %f", xmax_);
    RCLCPP_INFO(this->get_logger(), "ymax: %f", ymax_);
    RCLCPP_INFO(this->get_logger(), "resampleThreshold: %f", resampleThreshold_);
    RCLCPP_INFO(this->get_logger(), "llsamplerange: %f", llsamplerange_);
    RCLCPP_INFO(this->get_logger(), "llsamplestep: %f", llsamplestep_);
    RCLCPP_INFO(this->get_logger(), "lasamplerange: %f", lasamplerange_);
    RCLCPP_INFO(this->get_logger(), "lasamplestep: %f", lasamplestep_);
    RCLCPP_INFO(this->get_logger(), "ogain: %f", ogain_);
    RCLCPP_INFO(this->get_logger(), "useIMU: %s", use_imu_ ? "true" : "false");
    RCLCPP_INFO(this->get_logger(), "imuAngularWeight: %f", imu_weight_);
    RCLCPP_INFO(this->get_logger(), "lsigma: %f", lsigma_);
    RCLCPP_INFO(this->get_logger(), "srr: %f", srr_);
    RCLCPP_INFO(this->get_logger(), "srt: %f", srt_);
    RCLCPP_INFO(this->get_logger(), "str: %f", str_);
    RCLCPP_INFO(this->get_logger(), "stt: %f", stt_);
...
//        std::bind(&SlamGmapping::laserCallback, this, std::placeholders::_1));
    ...
    // Ìí¼Ó IMU ¶©ÔÄ
    if(use_imu_) {
        RCLCPP_INFO(this->get_logger(), "GMMapping: Using IMU data for orientation correction");
        // ¶©ÔÄIMU»°Ìâ
        imu_subscriber_ = this->create_subscription<sensor_msgs::msg::Imu>(
            "imu", 10, std::bind(&SlamGmapping::imuCallback, this, std::placeholders::_1));
    } else {
        RCLCPP_INFO(this->get_logger(), "GMMapping: Not using IMU data");
    }
}
...
bool SlamGmapping::initMapper(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan)
{
    ...
    // ÉèÖÃÉ¨ÃèÆ¥ÅäÆ÷µÄIMU²ÎÊý
    gsp_->m_matcher.setIMUParameters(use_imu_, imu_weight_);
    if(use_imu_ && got_imu_) {
        gsp_->m_matcher.updateIMUOrientation(imu_orientation_);
    }

    RCLCPP_INFO(this->get_logger(), "-----------------------Initialization complete %d",use_imu_);
...
bool SlamGmapping::addScan(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan, GMapping::OrientedPoint& gmap_pose) {
    if (!getOdomPose(gmap_pose, scan->header.stamp)){
        RCLCPP_WARN(this->get_logger(), "»ñÈ¡Àï³Ì¼ÆÎ»×ËÊ§°Ü!");
        return false;
    }

    if (scan->ranges.size() != gsp_laser_beam_count_){
        RCLCPP_ERROR(this->get_logger(), "¼¤¹âÉ¨ÃèµãÊý²»Æ¥Åä! ÆÚÍû: %d, Êµ¼Ê: %lu", 
    ...
    // ÔÚ´¦Àí¼¤¹âÉ¨ÃèÊ±Ê¹ÓÃIMUÊý¾Ý
    if(use_imu_ && got_imu_) {
        gsp_->m_matcher.setUseIMU(true);
        gsp_->m_matcher.updateIMUOrientation(imu_orientation_);
    }
...
void SlamGmapping::imuCallback(sensor_msgs::msg::Imu::ConstSharedPtr imu) {

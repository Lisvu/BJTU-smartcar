...
function bindHold(button) {
  ...
  const start = async (e) => {
    ...
    startedAt = Date.now();
    await move(kind);
    if (kind !== 'stop') timer = setInterval(() => move(kind), 180);
  };
  const stop = async () => {
    ...
    if (kind !== 'stop') {
      ...
      stopTimer = setTimeout(() => move('stop'), delay);
    }
  };
...
function init() {
  ...
  document.querySelectorAll('[data-start]').forEach((btn) => {
    btn.onclick = () => api('/api/process/start', { name: btn.dataset.start });
  });

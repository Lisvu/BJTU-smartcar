...
class yahboomcar_driver(Node):
	def __init__(self, name):
		...
		self.get_logger().info("car_type: {self.car_type}")
		self.declare_parameter('imu_link', 'imu_link')
		self.imu_link = self.get_parameter('imu_link').get_parameter_value().string_value
		self.get_logger().info("imu_link: {self.imu_link}")
		...
		self.get_logger().info("xlinear_limit: {self.xlinear_limit}")
		...
		self.get_logger().info("ylinear_limit: {self.ylinear_limit}")
		...
		self.get_logger().info("angular_limit: {self.angular_limit}")
		...
		#create timer
		self.timer = self.create_timer(0.04, self.pub_data)
		...
		self.accumulated_sum = [0, 0] # accumulated sum, counter
		self.alpha = 0.1 # low pass filter alpha
		self.prev_gz = 0.0 # previous gz value
	...
	def cmd_vel_callback(self,msg):
		...
		vy = msg.linear.y
		angular = msg.angular.z    # wait for chang
		self.car.set_car_motion(vx, vy, angular)
		# self.get_logger().info("cmd_vx: {} cmd_vy: {} cmd_angular: {}".format(vx, vy, angular))
	...
	def low_pass_filter(self, current_value, prev_value, alpha):
		"""
        Applies a first-order low-pass filter to the current value.
        """
	...
	def pub_data(self):
		...
		#imu.linear_acceleration.x = 0.0 if ax*1.0 > -0.2 and ax*1.0 < 0.2 else ax * 1.0
		imu.linear_acceleration.y = ay*1.0
		#imu.linear_acceleration.y = 0.0 if ay*1.0 > -0.16 and ay*1.0 < 0.16 else ay * 1.0
		imu.linear_acceleration.z = az*1.0
		#imu.angular_velocity.x = 0.0  if gx*1.0 > -0.005 and gx*1.0 < 0.002 else gx * 1.0
		#imu.angular_velocity.y = 0.0  if gy*1.0 > -0.002 and gy*1.0 < 0.002 else gy * 1.0
		imu.angular_velocity.x = gx * 1.0
		imu.angular_velocity.y = gy * 1.0
  
		# add a low pass filter to gz
		filtered_gz = self.low_pass_filter(gz, self.prev_gz, self.alpha)
		imu.angular_velocity.z = filtered_gz - 5e-4
		# Update previous gz value for the next cycle
		self.prev_gz = filtered_gz
	
		# imu.angular_velocity.z = gz * 1.0 - 5e-4
		#imu.angular_velocity.z = 0.0  if gz*1.0 > -0.03 and gz*1.0 < 0.005 else gz * 1.0
		# imu.angular_velocity.x = 0.0  if gx*1.0 > -0.003 and gx*1.0 < 0.0026 else gx * 1.0
		# imu.angular_velocity.y = 0.0  if gy*1.0 > -0.002 and gy*1.0 < 0.003 else gy * 1.0
		# imu.angular_velocity.z = 0.0  if gz*1.0 > -0.002 and gz*1.0 < 0.0025 else gz * 1.0

		# sum up 
		self.accumulated_sum[0] += gz
		self.accumulated_sum[1] += 1
		self.get_logger().info(
			"acc sum: {}, counter: {}, avg: {}".format(self.accumulated_sum[0], self.accumulated_sum[1], self.accumulated_sum[0]/self.accumulated_sum[1]))
